...
def btb_test(nums, aligns, name):
    ...
    for align in aligns:
        ...
        core.append([])
        for num in nums:
            print("Running num=", num, "and align=", align)
            res = btb_size_test("BTB size test %d branches aligned on %d" % (num, align), num, align)
            exp = num * 4 * 100.0 # number of branches under test
...
def btb_plot(nums, aligns, name, results, alt):
    ...
    if alt:
        ...
        plot(nums, aligns, results['early'], "Early BPU clears", 2)
        plot(nums, aligns, results['late'], "Late BPU clears", 3)
...
def add_tests(agner):
    # attempt to find cache set collision
    add_test(agner, [1,2,3,4,5], [2**x for x in range(2, 19)], "Set collision")

    # Detailed test with only one case for executable analysis for addresses
    #add_test(agner, [4], [2**x for x in range(12, 13)], "Set collision")
