...
def btb_unconditional_test(name, num_branches, align):
    test_code = """
%macro OneJump 0
jmp %%end
align {align}/4
...
def btb_test(nums, aligns, name):
    ...
    for align in aligns:
        ...
        for num in nums:
            ...
            res = btb_unconditional_test("BTB loop test %d branches aligned on %d" % (num, align), num, align)
            exp = num * 100.0 * 5 # number of branches under test
...
def add_tests(agner):
    add_test(agner, range(1,12), [2**x for x in range(2, 19)], "Loop test")
